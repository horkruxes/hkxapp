...
const CURVE = {
  ...
  n: BigInt(2 ** 252 + 27742317777372353535851937790883648493),
  // Cofactor
  h: 8n,
...
class ExtendedPoint {
  ...
  double(): ExtendedPoint {
    ...
    const Z1 = this.z;
    const { a } = CURVE;
    const A = BigInt(mod(BigInt(X1) ** BigInt(2)));
    const B = mod(Y1 ** 2n);
...
class Point {
  ...
  static fromHex(hash: Hex) {
    ...
    const normed = Uint8Array.from(
      Array.from(bytes.slice(0, 31)).concat(normedLast)
    );
...
export async function verify(
  ...
  const hs = await sha512ToNumberLE(
    signature.r.toRawBytes(),
...
export const utils = {
  ...
  randomBytes: (bytesLength: number = 32): Uint8Array => {
    return Random.getRandomBytes(bytesLength);
  },
  ...
  randomPrivateKey: (): Uint8Array => {
    ...
    throw new Error(
      "Valid private key was not found in 1024 iterations. PRNG is broken"
    );
  },
  sha512: async (message: Uint8Array): Promise<Uint8Array> => {
    ...
    let diget = await Crypto.digestStringAsync(
      Crypto.CryptoDigestAlgorithm.SHA256,
      dec.decode(message)
    );
    return enc.encode(diget);
  },
